...
WALL_MODEL_ID :: -2
ACTOR_POSITION_Y :: 0.0
ACTOR_GRAVEYARD_POSITION :: 999.0
ACTOR_MAX_DISTANCE_FROM_PLAYER :: 1.25
ACTOR_DEFAULT_MOVEMENT_SPEED :: 0.45 // Lower values result to lower speed
ACTOR_DEFAULT_ROTATION_SPEED :: 3.0
ACTOR_DEFAULT_ANIMATION_SPEED :: 30.0 // Animation played in FPS
ITEM_START_POSITION_Y :: 0.1
...
CreateRay :: proc(entity : ^Entity) -> rl.Ray {
	v := rl.Vector3Normalize(rl.Vector3Subtract(entity.transform.position, player.Player.transform.position))
...
TestPlayerHit :: proc(entity : ^Entity) -> bool {
	...
	if (rl.Vector3Distance(player.Player.transform.position, entity.transform.position) > 5.0 && entity.data.value.(Actor).playerSpotted) {
		return false;
	}
...
UpdatePosition :: proc(entity : ^Entity) -> bool {
	...
	distanceFromPlayer : rl.Vector3 : rl.Vector3Subtract(entity.transform.position, player.Player.transform.position)
		...
		libc.fabsf(distanceFromPlayer.z) >= ACTOR_MAX_DISTANCE_FROM_PLAYER)
	{
		...
		entityNewPosition : rl.Vector3 : {player.Player.transform.position.x, ACTOR_POSITION_Y, player.Player.transform.position.z}
		entity.transform.position := rl.Vector3Lerp(entity.transform.position, entityNewPosition, entity.data.value.(Actor).movementSpeed * GetFrameTime())
